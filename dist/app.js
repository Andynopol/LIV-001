...
class CrossWrodsEditor
{
	...
	getVerticalCellCoords ( cell )
	{
		const coords = [];
		const row = cell.parentElement.parentElement;
		const blanks = [ ...row.getElementsByClassName( 'blank' ) ];
		coords.push( this.rows.indexOf( row ) );
		coords.push( blanks.indexOf( cell ) );
		return coords;
	}
	...
	markVertical ( coords )
	{
		...
		for ( var i = 0; i < this.rows.length; i++ )
		{
			const cells = [ ...this.rows[ i ].getElementsByClassName( 'blank' ) ];
			for ( var j = 0; j < cells.length; j++ )
			{
				cells[ j ].classList.remove( 'vertical' );
				if ( coords[ 1 ] === j && cells[ j ].classList.contains( 'cell' ) )
				{
					cells[ j ].classList.add( 'vertical' );
				}
			}
		}
	}
	...
	verticalMover ( btn, coords, first, last )
	{
		...
		if ( coords[ 1 ] === first )
		{
			this.vertLeft.classList.add( 'disabled' );
		}
		if ( coords[ 1 ] > first && this.vertLeft.classList.contains( 'disabled' ) )
		{
			this.vertLeft.classList.remove( 'disabled' );
		}
		if ( coords[ 1 ] < last && this.vertRight.classList.contains( 'disabled' ) )
		{
